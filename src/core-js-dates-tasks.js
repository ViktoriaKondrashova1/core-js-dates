...
function getTime(date) {
  ...
  const hours = getNumber(dateObj.getHours());
  const minutes = getNumber(dateObj.getMinutes());
  const seconds = getNumber(dateObj.getSeconds());
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const timeSart = new Date(dateStart).getTime();
  const timeEnd = new Date(dateEnd).getTime();
  const msInDay = 1000 * 60 * 60 * 24;
  return (timeEnd - timeSart) / msInDay + 1;
}
...
function formatDate(date) {
  ...
  return `${month + 1}/${day}/${year}, ${formattedHours}:${minutes}:${seconds} ${timeOfDay}`;
}
...
function getWeekNumberByDate(date) {
  ...
  return 1 + Math.ceil((firstThursday - date) / ms);
}
...
function isLeapYear(date) {
  ...
  const lastFedDay = new Date(year, 2, 0).getDate();
  return lastFedDay === 29;
}
